...
export class SwapFactory extends Entity {
  ...
  static load(id: string): SwapFactory | null {
    return changetype<SwapFactory | null>(store.get("SwapFactory", id));
  }
...
export class SpookyswapDayData extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save SpookyswapDayData entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save SpookyswapDayData entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("SpookyswapDayData", id.toString(), this);
    }
  }

  static load(id: string): SpookyswapDayData | null {
    return changetype<SpookyswapDayData | null>(
      store.get("SpookyswapDayData", id)
    );
  }
